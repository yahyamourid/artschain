            ...
            <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center z-50  backdrop-blur-sm">
                <div className="relative flex-col h-5/6 w-2/6 bg-slate-950 border border-opacity-20 border-gray-500 rounded-lg flex items-center justify-start shadow-xl ">
                    ...
                    <div className="mb-4  w-3/5">
                        <label htmlFor="name" className="flex flex-col text-sm  text-white mb-1 font-medium tracking-wider">UserName:</label>
                        <input id="name" type="text" placeholder="Choose a username" onChange={handleUsername} value={userName}
                            className="text-white font-normal w-full text-sm bg-slate-950  border border-opacity-20 border-gray-500 hover:bg-gray-900 rounded-lg  px-4 py-2 duration-300 focus:outline-none focus:border-opacity-35 focus:bg-gray-900 " />
                        {error && <p className='text-red-500 text-sm'>userName already exist</p>}
                    </div>
                    <div className="mb-4  w-3/5">
                        <label htmlFor="name" className="flex flex-col text-sm  text-white mb-1 font-medium tracking-wider">FirstName:</label>
                        <input id="firstname" type="text" placeholder="Your first" onChange={handleFirstName} value={firstName}
                            className="text-white font-normal w-full text-sm bg-slate-950 border border-opacity-20 border-gray-500 hover:bg-gray-900 rounded-lg  px-4 py-2 duration-300 focus:outline-none focus:border-opacity-35 focus:bg-gray-900 " />
                    </div>
                    <div className="mb-4  w-3/5">
                        <label htmlFor="name" className="flex flex-col text-sm  text-white mb-1 font-medium tracking-wider">LastName:</label>
                        <input id="firstname" type="text" placeholder="Your lastName" onChange={handleLastName} value={lastName}
                            className="text-white font-normal w-full text-sm bg-slate-950 border border-opacity-20 border-gray-500 hover:bg-gray-900 rounded-lg  px-4 py-2 duration-300 focus:outline-none focus:border-opacity-35 focus:bg-gray-900 " />
                    </div>
                    <div className="mb-4 w-3/5">
                        <label htmlFor="walletAdress" className="block text-sm  text-white mb-1 font-medium tracking-wider">WalletAdress:</label>
                        <input id="walletAdress" type="text" value={walletAddress} readOnly 
                            className="text-white font-normal text-sm w-full bg-gray-900 border border-opacity-20 border-gray-500  rounded-lg  px-4 py-2 duration-300 focus:outline-none focus:border-opacity-35 focus:bg-gray-900 " />
                    </div>
                    <button className={`w-3/5 ${activeButton ? 'bg-violet-600 hover:bg-gray-900 ' : 'bg-gray-800'} text-white tracking-widest border border-opacity-20 border-gray-500  rounded-lg  px-4 py-1.5 duration-300`}
                     disabled={!activeButton} onClick={()=> register(userName, firstName, lastName)}>Register</button>
