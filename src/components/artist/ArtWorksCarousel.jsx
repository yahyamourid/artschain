        ...
        <div className='relative flex items-center justify-center h-1/2 w-full py-6 gap-5'>
            <div className='flex gap-4 overflow-x-auto w-full h-full'>
                {artWorks.slice(startIndex, startIndex + 4).map((artwork, index) => (
                    <div key={index} className='group flex border border-violet-600 items-end justify-center w-1/4 h-full cursor-pointer rounded-lg p-4 hover:scale-95 duration-500 ' style={{ backgroundImage: `url(https://gateway.pinata.cloud/ipfs/${artwork.imageUrl})`, backgroundSize: 'cover' }}
                        ...
                        <span className='flex text-xs text-zinc-400 font-medium flex-col group-hover:opacity-0 duration-500 justify-start w-full bg-opacity-70 bg-zinc-950 py-1 pl-1 gap-0.5 rounded-tr-2xl rounded-bl-2xl'>
                            ...
                            <p>{artwork[9]} x {artwork.width}</p>
                        </span>
                    </div>
                ))}
            </div>
            <button onClick={handleNext} className={`absolute right-0 top-1/2 transform -translate-y-1/2 text-3xl hover:text-pink-700 duration-300 text-white px-3 py-1 rounded-lg ${startIndex + 4 >= artWorks.length && 'opacity-50 cursor-not-allowed'}`} disabled={startIndex + 4 >= artWorks.length}>
                <BsArrowRightCircleFill />
            </button>
            <button onClick={handlePrev} className={`absolute left-0 top-1/2 transform -translate-y-1/2 text-3xl hover:text-pink-700 duration-300 text-white px-3 py-1 rounded-lg ${startIndex === 0 && 'opacity-50 cursor-not-allowed'}`} disabled={startIndex === 0}>
